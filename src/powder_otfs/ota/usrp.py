...
def receive_samples(
    ...
    """Capture a fixed number of complex samples."""
    ...
    rx_streamer.issue_stream_cmd(stream_command)
    ...
    while offset < num_samples:
        ...
        if metadata.error_code != (
            ...
            raise RuntimeError(
                f"USRP receive error: {metadata.strerror()}"
            )
        if count == 0:
            raise TimeoutError(
                "No samples were received from the USRP."
            )

        received[offset:offset + count] = buffer[0, :count]
        offset += count
